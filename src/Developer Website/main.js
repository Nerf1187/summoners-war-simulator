...
window.onload = () => {
    ...
    addAbilityButton.addEventListener("click", () => {
        ...
        newDiv.innerHTML = `
                ...
                <h2 id="${divName}-header">Ability ${divNum}</h2>
                ...
                <button type="button" class="debuff-button" onclick="{
                    this.parentElement.parentElement.renameAllChildrenEffects(Number.parseInt(this.parentElement.id.charAt(7)), true);
    ...
    function formatCode()
    {
        ...
        let statement = "super(\"{name}\" + count, {element}, {hp}, {def}, {attack}, {spd}, {critRate}, {critDmg}, {res}, {acc});".formatUnicorn({
            name:     name,
        ...
        return `        package Monsters.${get("element").toTitleCase()};
        ...
        public class ${name} extends Monster
        {
            ...
            */
            ...
            {
                ${statement}
                super.setRunes(MonsterRunes.getRunesFromFile("runeFileName.csv", this));
                setAbilities();
                count++;
            }
     ...
     * @return {boolean | String | Number | null} The element value, or false if the element does not exist
     */
    ...
    function formatAllAbilitiesCode()
    {
        ...
        //Add final line;
        code += "super.setAbilities(abilities);";
    ...
    function formatAbilityCode(abilityNum, stringNum)
    {
        ...
        switch (type)
        {
            ...
            case "Parent":
                ...
                break;
            case "Leader":
                code += "Stat.{stat}, {amount}, {element}".formatUnicorn({
    ...
    function formatDebuffs(abilityNum)
    {
        ...
        while (true)
        {
            ...
            if (get(`${abilityName}-debuff${count}-selector`))
            {
                ...
                if (!get(`${abilityName}-debuff${count}-amount`))
                {
                    ...
                    line1Code += "Debuff.{debuff}, {turns}, {throughImmunity}".formatUnicorn({
                ...
                {
                    line2Code = `\t\tArrayList<Integer> ${abilityName}DebuffChances = abilityChances(`;
                }
        ...
        return formatStatLines(line1Code, line2Code, extraLines);
    }
    ...
    function formatBuffs(abilityNum)
    {
        ...
        while (true)
        {
            ...
            if (get(`${abilityName}-buff${count}-selector`))
            {
                ...
                {
                    line1Code = `ArrayList<Buff> ${abilityName}Buffs = abilityBuffs(`;
                }
                
                if (!get(`${abilityName}-buff${count}-amount`))
                {
                    //Write buff info to line 1
                    line1Code += "Buff.{buff}, {turns}".formatUnicorn({
                ...
                {
                    line2Code = `\t\tArrayList<Integer> ${abilityName}BuffChances = abilityChances(`;
                }
     ...
     */
    function formatStatLines(line1, line2, extraLines)
    {
        //End of lines
        if (line1)
        {
 ...
 * @returns {Number | NaN} The parsed number If the String contains any characters that are not numeric or contains a dash that is not the first character,
 * return NaN
 */
...
{
    fetch("../Stats/Buffs/Buff key.csv").then((res) => res.blob()).then((res) => {
...
{
    fetch("../Stats/Debuffs/Debuff key.csv").then((res) => res.blob()).then((res) => {
        res.text().then((res) => res.split("\n").forEach(buff => {
...
/**
 * Renames all children of the HTMLElement to decrease the effect number by one if the current number is greater than oldNum
 ...
 */
HTMLElement.prototype.renameAllChildrenEffects = function (oldNum, decAbility = false) {
    for (let element of this.children)
    {
        //If this is a div, rename the children
        if (element.tagName.toLowerCase() === "div")
        {
            element.renameAllChildrenEffects(oldNum, decAbility);
        }
        //Do nothing if this is a label
        ...
        let index = (!decAbility) ? newId.indexOf("buff") + "buff".length : "ability".length;
        let num = -1;
        let i;
        ...
        for (i = 1; i <= 3; i++)
        {
            ...
            let temp = Number.parseInt(newId.substring(index, index + i));
            if (isNaN(temp))
            {
                i--;
                break;
        ...
        //Do nothing if the current number is already lower than the old one
        if (num < oldNum)
        {
            continue;
        }
        ...
        //Create the new ID
        newId = newId.substring(0, index) + (num - 1) + newId.substring(index + i);
        //Reset the elements ID
        element.id = newId;
        if (element.tagName.toLowerCase().includes("h"))
        {
            element.innerHTML = newId.substring((element.tagName.includes("4")) ? "ability_-".length : 0, index).toTitleCase() + " " + newId.charAt(index);
        }
    }
};