...
public class Hekerson extends Monster
{
    ...
    public Hekerson(String fileName)
    {
        this();
    ...
    private void setAbilities()
    {
        ...
        abilities.add(new Attack_Ability("Concussive Shot (1)", 4.1 * 1.2, 0, 1, "Attacks the enemy and " +
                "Stuns the target for 1 turn with a 50% chance.", ability1Debuffs, ability1DebuffChances, 0, false,
                false));
        ...
        abilities.add(new Attack_Ability("Ruthless Bombing (3)", 1.6 * 1.15, 0, 3, "Attacks all enemies 3" +
                ...
                "HP " +
                "by 50% of the damage dealt.", ability3Debuffs, ability3DebuffChances, 4, false, false));
    ...
    public boolean nextTurn(Monster target, int abilityNum)
    {
        ...
        {
            target.destroyHp((int) (getDmgDealtThisTurn() * 0.5));
            Team other = (game.getOtherTeam().size() > 0) ? game.getOtherTeam() : Auto_Play.getOther();
            for (int i = 0; i < other.size(); i++)
            {
                Monster m = other.get(i);
                ...
                m.destroyHp((int) (getDmgDealtThisTurn() * 0.5));
            }
        }
