...
public class Aegir extends Monster
{
    ...
    private void setAbilities()
    {
        ...
        abilities.add(new Attack_Ability("Wrathful Attack (2)", 6.2 * 1.25, 0, 1,
                ...
                ability2DebuffChances, 2, false, false, false));
        ...
        abilities.add(new Attack_Ability("Confiscate (3)", 4 * 1.25, 0, 2, "Attacks the enemy 2 times, with each strike having a 75% chance to steal 1 beneficial effect from the enemy. Absorbs the Attack Bar by 50% each if you attack the " +
                "enemy with no beneficial effects. Goes under [Berserk] state for 3 turns afterwards. Under Berserk state, the DEF is decreased by 30%, damage dealt to enemies is decreased by 30%, damage dealt to enemies is "
                + "increased by 100%, Attack Speed is increased by 20% and HP is recovered by 10% of the damage dealt.", ability3Buffs, ability3BuffsChances, 3, false, false, 0, false));
    ...
    public boolean nextTurn(Monster target, int abilityNum)
    {
        ...
        if (berserk.getNumTurns() <= 0)
        {
            ...
            setCurrentHp((int) ((getCurrentHp() * 10.0 / 7) + 0.5));
        }
